...
async function displayUsageStats() {
  ...
  try {
    ...
    if (stats) {
      ...
      console.log(`Total Cost: $${(Number(stats.total_cost) || 0).toFixed(4)}`);
      console.log(`Avg Tokens/Message: ${Math.round(Number(stats.avg_tokens_per_message) || 0)}`);
      console.log(`Avg Cost/Message: $${(Number(stats.avg_cost_per_message) || 0).toFixed(6)}`);
    ...
    if (modelUsage && modelUsage.length > 0) {
      ...
      for (const model of modelUsage) {
        ...
        console.log(`  Total Cost: $${(Number(model.total_cost) || 0).toFixed(4)}`);
        console.log(`  Avg Processing Time: ${Math.round(Number(model.avg_processing_time) || 0)}ms`);
      }
    }
    ...
    if (topChannels && topChannels.length > 0) {
      ...
      for (let i = 0; i < topChannels.length; i++) {
        ...
        console.log(`   Total Cost: $${(Number(channel.total_cost) || 0).toFixed(4)}`);
        console.log(`   Avg Sentiment: ${(Number(channel.avg_sentiment) || 0).toFixed(2)}`);
      }
    }
    ...
    if (stats && stats.total_messages > 0) {
      const daysSinceFirst = stats.first_message 
        ...
        : 1;
      const dailyAvgCost = (Number(stats.total_cost) || 0) / Math.max(daysSinceFirst, 1);
